...
- **Test mocking pattern** — mock `Invoke-GitlabApi`, `New-GitlabObject`, and resolver functions. Private helpers needed by the module under test (e.g. `Globals.ps1`, `PaginationHelpers.ps1`) must be included _inside_ the `New-Module` scriptblock, not dot-sourced outside it — modules only see their own scope and global. See `tests/Get-GitlabMergeRequest.Tests.ps1` for the pattern.
