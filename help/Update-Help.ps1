...
Write-Host "Reformatting syntax blocks..." -ForegroundColor Cyan
Get-ChildItem -Path $DocsFolder -Filter '*.md' -Recurse | ForEach-Object {
    $Content = Get-Content $_.FullName -Raw
    $Updated = $Content

    # Replace "__AllParameterSets" with "Default" for cleaner display
    $Updated = $Updated -replace '### __AllParameterSets', '### Default'

    # Reformat syntax blocks: one parameter per line
    $Updated = [regex]::Replace($Updated, '(### (?:Default|\w+)\s*\r?\n\r?\n```\r?\n)([\s\S]*?)(```)', {
        param($m)
        $before = $m.Groups[1].Value
        $syntax = $m.Groups[2].Value
        $after = $m.Groups[3].Value

        # Normalize whitespace
        $normalized = ($syntax -replace '\s+', ' ').Trim()

        # Match cmdlet name and parameters
        if ($normalized -match '^([A-Z][a-zA-Z]+-[a-zA-Z]+)\s+(.*)$') {
            $cmdlet = $Matches[1]
            $params = $Matches[2]

            # Split parameters on "] [" pattern
            $paramList = $params -split '\]\s+\[' | ForEach-Object {
                $p = $_.Trim()
            ...
            return "$before$cmdlet`n$($paramList -join "`n")`n$after"
        }
        return $m.Value
    })
    ...
    if ($Updated -ne $Content) {
        ...
        Write-Debug "  Reformatted: $($_.Name)"
    }
}
...
foreach ($Category in $Categories) {
    ...
    if ($CmdletFiles) {
        # Add category header (collapsible link format for docsify-sidebar-collapse)
        $SidebarContent += "* [$($Category.Name)]()"
...
# Build the categories list (link to first cmdlet in each category)
$CategoriesList = @()
foreach ($Category in $Categories) {
    $Description = $CategoryDescriptions[$Category.Name]
    if (-not $Description) { $Description = "Manage $($Category.Name)" }
    
    # Get the first cmdlet file to link to
    $FirstCmdlet = Get-ChildItem -Path $Category.FullName -Filter '*.md' | Sort-Object BaseName | Select-Object -First 1
    if ($FirstCmdlet) {
        $LinkPath = "$($Category.Name)/$($FirstCmdlet.Name)"
        $CategoriesList += "- [$($Category.Name)]($LinkPath) - $Description"
    }
}
$CategoriesText = $CategoriesList -join "`n"

# Replace the categories section in README
$Pattern = '(?s)(## Cmdlet Categories\s+Browse the sidebar to find cmdlets organized by category:\s+)(- \[.*?)(\s+## Links)'
