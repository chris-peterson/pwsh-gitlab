...
Update-GitlabLabel [[-ProjectId] <string>] [-LabelId] <int> [[-NewName] <string>] [[-Color] <string>]
 [[-Description] <string>] [[-Priority] <Nullable`1>] [[-SiteUrl] <string>] [-WhatIf] [-Confirm]
 [<CommonParameters>]
```
...
Updates properties of an existing label in a GitLab project or group. You can change the name, color, description, or priority.
...
```powershell
Update-GitlabLabel -LabelId 42 -NewName 'critical-bug'
```

Renames the label with ID 42 in the current project.

### Example 2: Update a label's color and description

```powershell
Update-GitlabLabel -ProjectId 'mygroup/myproject' -LabelId 42 -Color '#00FF00' -Description 'Updated description'
```

Updates the color and description of the specified label.

## PARAMETERS

### -Color

The new color for the label in hex format (e.g. '#00FF00').
...
The new priority for the label. Use $null to remove priority.
...
- [GitLab Labels API](https://docs.gitlab.com/ee/api/labels.html#edit-an-existing-label)
