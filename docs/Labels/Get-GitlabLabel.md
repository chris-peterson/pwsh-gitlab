...
Retrieves labels from a GitLab project or group. Can list all labels or filter by name or search term. Supports both project-level and group-level labels.
...
Retrieves all labels defined in the specified group.

### Example 3: Search for labels by name

```powershell
Get-GitlabLabel -ProjectId 'mygroup/myproject' -Search 'bug'
```

Searches for labels containing 'bug' in the specified project.

## PARAMETERS

### -All

If specified, retrieves all labels by fetching all pages of results. By default, only a limited number of results are returned.
...
The exact name of a label to retrieve. Filters client-side after fetching labels.
...
A search term to filter labels by name server-side.
